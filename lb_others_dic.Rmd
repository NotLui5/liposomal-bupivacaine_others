...
<summary><b>Figure 1 -</b> Adverse effects of liposomal bupivacaine vs others</summary>
<br>
...
col1f <- c("Author", datad_2ae$Author, 
      ...
      paste("(Tau^2 = ", (formatC(meta_datga_reml$tau2, digits=2, format="f")), ", df = ", 
            ...
            "; ", "I^2", " = ", (formatC(meta_datga_reml$I2, digits=1, format="f")), "%)"),NA, NA,
      "Author", datad_rm$Author, 
      paste("Overall Odds Ratio for", meta_datrm_reml$k, "studies"),
      paste("(Tau^2 = ", (formatC(meta_datrm_reml$tau2, digits=2, format="f")), ", df = ", 
            (meta_datrm_reml$parms),
            ", p ", (ifelse(meta_datrm_reml$QEp < 0.001, 
                            paste("< 0.001"),
                            paste("= ", formatC(meta_datrm_reml$QEp, digits=3, format="f")))),
            "; ", "I^2", " = ", (formatC(meta_datrm_reml$I2, digits=1, format="f")), "%)"))
...
col2f <- c("Liposomal Bupivacaine (n/N)",
      ...
      NA, NA, 
      "Liposomal Bupivacaine (n/N)",
      paste(dat_rm$e1,"/", dat_rm$n1), paste0(sum(dat_rm$e1), " / ", sum(dat_rm$n1)), NA)
...
col3f <- c("others (n/N)",
      ...
      NA, NA,
      "others (n/N)",
      paste(dat_rm$e2,"/", dat_rm$n2), paste0(sum(dat_rm$e2), " / ", sum(dat_rm$n2)), NA)
...
col4f <- c("Odds Ratio (95% CI)", paste0(formatC(exp(dat_2ae$yi), digits = 2, format = "f"), 
          ...
          NA, NA,
          "Odds Ratio (95% CI)", paste0(formatC(exp(dat_rm$yi), digits = 2, format = "f"), 
                                        " ( ",formatC(exp(dat_rm$ci.lb), format='f', digits=2),
                                        " - ", formatC(exp(dat_rm$ci.ub), format='f',digits=2), " ) "),
          paste(formatC(meta_rm$pred, format = 'f', digits=2),
            "(", formatC(meta_rm$ci.lb, format = 'f', digits = 2),
            "-", formatC(meta_rm$ci.ub, format = 'f', digits = 2), ")"), NA)
...
col5f <- c("Weight(%)", paste(formatC(weights(meta_dat2ae_reml), format='f', digits = 1),'%'), NA, NA,
           ...
           NA, NA,
           "Weight(%)", paste(formatC(weights(meta_datrm_reml), format='f', digits = 1),'%'), NA, NA)
...
subtitle6 <- c("Analgesic rescue medication 12h post", rep(NA,4))

text1f <- rbind(text1f[1,], subtitle1, text1f[2:7,], subtitle2, text1f[c(9:15),],
                subtitle3, text1f[c(17:22),], subtitle4, text1f[c(24:29),],
                subtitle5, text1f[c(31:36),], subtitle6, text1f[c(38:41),])
rownames(text1f) <- NULL


cf <- structure(list(
          mean = c(rep(NA, 2), exp(dat_2ae$yi), meta_2ae$pred, rep(NA, 4), exp(dat_vn$yi), meta_vn$pred, rep(NA, 4), exp(dat_p$yi), meta_p$pred, rep(NA, 4), exp(dat_ah$yi), meta_ah$pred, rep(NA, 4), exp(dat_ga$yi), meta_ga$pred, rep(NA, 4), exp(dat_rm$yi), meta_rm$pred,NA),
          lower = c(rep(NA, 2), exp(dat_2ae$ci.lb), meta_2ae$ci.lb, rep(NA, 4), exp(dat_vn$ci.lb), meta_vn$ci.lb, rep(NA, 4), exp(dat_p$ci.lb), meta_p$ci.lb, rep(NA, 4), exp(dat_ah$ci.lb), meta_ah$ci.lb, rep(NA, 4), exp(dat_ga$ci.lb), meta_ga$ci.lb, rep(NA,4), exp(dat_rm$ci.lb), meta_rm$ci.lb, NA),
          upper = c(rep(NA, 2), exp(dat_2ae$ci.ub), meta_2ae$ci.ub, rep(NA, 4), exp(dat_vn$ci.ub), meta_vn$ci.ub, rep(NA, 4), exp(dat_p$ci.ub), meta_p$ci.ub, rep(NA, 4), exp(dat_ah$ci.ub), meta_ah$ci.ub, rep(NA, 4), exp(dat_ga$ci.ub), meta_ga$ci.ub, rep(NA,4), exp(dat_rm$ci.ub), meta_rm$ci.ub, NA)),
...
sizes1f <- c(NA, NA, (weights(meta_dat2ae_reml)*0.014), 1, rep(NA, 3),
             ...
             NA, (weights(meta_datga_reml)*0.014), 1, rep(NA, 3), 
             NA, (weights(meta_datrm_reml)*0.014), 1, NA)
names(sizes1f) <- NULL
